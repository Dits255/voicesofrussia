...
Направление дизайна — **«Лента голосов»**: люди-первыми, крупные портретные карточки, мультиформат
(лонгрид · интервью · подборка · видео · подкаст).
...
(`isPlaceholder: true` в `content.json`, помечены бейджем «скоро»). Чтобы добавить реальное видео:
заменить запись в `content.json` (убрать `isPlaceholder`, добавить ссылку/плеер) и при желании
расширить страницу `Story`.
